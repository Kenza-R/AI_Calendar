#!/usr/bin/env python3
"""
Enhanced Assessment Parser - API Ready
...
def extract_assessment_components_api(text_content: str) -> Dict:
    """
    API-ready function to extract assessment components from syllabus text.
    ...
    try:
        ...
        components = extract_assessment_components(text_content)
        
        return {
            "success": True,
            "components": components,
            "count": len(components),
            "total_weight": sum(c.get("weight_percent", 0) for c in components)
        }
...
def extract_from_file_api(file_path: Path) -> Dict:
    """
    API-ready function to extract assessment components from a file.
    ...
    try:
        ...
        return extract_assessment_components_api(text)
