"""
Enhanced deadline & class-session extraction from PDF files - API Ready
Uses a snippet-based approach for reliability across different syllabi.
Integrated version for backend API endpoints.
"""
import sys
import os
...
def extract_deadlines_and_sessions_api(
    text_content: str,
    assessment_components: Optional[List[Dict]] = None
) -> Dict:
    """
    API-ready function to extract deadlines and class sessions from syllabus text.
    
    Args:
        text_content: The full text extracted from the document
        assessment_components: Optional list of assessment components for context
    ...
    try:
        ...
        return {
            ...
            "items": items,
            "count": len(items),
            "hard_deadlines": [item for item in items if item.get("kind") == "hard_deadline"],
            "class_sessions": [item for item in items if item.get("kind") == "class_session"]
        }
...
def extract_from_file_api(file_path: Path) -> Dict:
    """
    API-ready function to extract deadlines from a file.
    ...
    try:
        ...
        # Try to get assessment components first
        assessment_components = None
        try:
            from app.utils.test_assessment_parser_copy import extract_assessment_components_api
            result = extract_assessment_components_api(text)
            if result.get("success"):
                assessment_components = result.get("components", [])
        except Exception:
            pass
        
        # Extract deadlines
        return extract_deadlines_and_sessions_api(text, assessment_components)
